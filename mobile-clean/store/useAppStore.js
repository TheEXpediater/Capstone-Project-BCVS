...
export const useAppStore = create((set, get) => ({
  ...
  async bootstrap() {
    ...
    const credentials = await vcService.listCredentials();
  ...
  async saveCredential(credential) {
    ...
    await get().addActivity({
      ...
      body: saved.meta?.title || 'Saved to wallet',
  ...
  async loadVerificationRequest(sessionId) {
    get().setLoading('verification', true);
    try {
      const request = await verificationService.getVerificationRequest(sessionId);
  ...
  async approveVerificationRequest({ sessionId, credential }) {
    get().setLoading('verification', true);
    try {
  ...
  async denyVerificationRequest(sessionId) {
    get().setLoading('verification', true);
    try {
      const result = await verificationService.denyVerificationRequest(sessionId);
      await get().addActivity({
