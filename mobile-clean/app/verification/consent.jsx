...
export default function ConsentScreen() {
  ...
  useEffect(() => {
    ...
    if (sessionId) {
      loadRequest(String(sessionId)).catch((error) => {
        Alert.alert('Request unavailable', error.message);
      });
    }
  }, [sessionId, loadCredentials, loadRequest]);

  const selectedCredential = useMemo(
    () => credentials.find((item) => String(item.id) === String(selectedId)) || credentials[0],
