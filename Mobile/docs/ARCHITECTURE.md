# BVCS Mobile Architecture

This document explains the clean rebuild for the Expo React Native mobile app. The goal is a simple holder credential app that is easy to explain during capstone defense.
...
- The root layout owns too much behavior: Redux provider, credential bootstrap, session polling, global consent modal, toast setup, and navigation all live in one file.
- State is split across Redux slices, Zustand credential store, AsyncStorage, refs, and component state. This makes bugs hard to trace.
...
1. Create `Mobile/` and keep the old mobile app as read-only reference.
...
5. Replace the old credential store with `vcService.js` plus `useAppStore.js`.
...
10. Run the clean app and test only five defense-critical flows: auth, credentials, scan, consent, notifications.
