...
export default function CredentialDetailScreen() {
  ...
  const blockchainAnchor = useMemo(
    () => getBlockchainAnchorInfo(credential),
    [credential]
  );

  async function openBlockchainAnchor() {
    if (!blockchainAnchor.url) return;

    try {
      await Linking.openURL(blockchainAnchor.url);
    } catch {
      Alert.alert('Unable to open link', 'The blockchain anchor could not be opened.');
    }
  }
    ...
    return (
      <Screen>
        ...
        <Button
          ...
          onPress={() => router.replace('/(tabs)/credentials')}
        />
      </Screen>
    );
  }
    ...
    <Screen padded={false}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.card}>
          ...
          {blockchainAnchor.url ? (
            <Pressable
              accessibilityRole="link"
              hitSlop={8}
              onPress={openBlockchainAnchor}
            >
              <Text style={styles.link}>{blockchainAnchor.label}</Text>
            </Pressable>
          ) : (
